...
PM2가 설치되어 있지 않다면, 다음 중 선호하는 패키지 매니저를 사용하여 전역(Global)으로 설치하세요.
...
> **참고**: 설치 없이 일회성으로 실행하고 싶다면 `bunx pm2` 또는 `npx pm2`를 명령어 앞에 붙여 사용할 수도 있으나, 데몬 관리를 위해 전역 설치를 권장합니다.
...
# 대시보드 형태의 모니터링 (CPU, 메모리, 로그 실시간 확인)
pm2 monit
```

### 로그 관리 (파일 및 초기화)
PM2는 로그를 파일로 저장합니다. 로그 파일 위치를 확인하거나 초기화할 수 있습니다.
```bash
# 로그 파일 위치 확인
pm2 show queue-worker

# 저장된 로그 비우기 (용량 확보)
pm2 flush queue-worker
```
