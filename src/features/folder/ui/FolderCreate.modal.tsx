...
function InviteCodeSuccessModal({ 
  ...
  return (
    <div className="fixed inset-0 z-[130] bg-black/50 flex items-center justify-center p-4" onClick={onClose}>
      <div 
        className="bg-white dark:bg-surface-900 rounded-3xl w-full max-w-sm p-6 flex flex-col items-center gap-6 shadow-2xl"
...
interface FolderCreateModalProps {
  onClose: () => void;
  onSuccess?: (data: CreateFolderResult) => void;
}

export function FolderCreateModal({ onClose, onSuccess }: FolderCreateModalProps) {
  const navigate = useNavigate();
  ...
  return createPortal(
    <div className="fixed inset-0 z-[120] flex items-center justify-center">
      ...
      <div className={cn(
        ...
        "md:max-w-md md:h-[90vh] md:rounded-[24px] md:overflow-hidden shadow-2xl"
      )}>
        {/* 헤더 */}
        ...
        <form 
          ...
          className="flex-1 overflow-y-auto p-6 flex flex-col gap-8 scrollbar-hide pb-safe"
        ...
        >
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-2">
              ...
              <textarea 
                ...
                maxLength={50}
                className="w-full p-4 rounded-xl bg-surface-50 dark:bg-surface-900 border border-surface-100 dark:border-surface-800 focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all resize-none h-24 text-sm"
              />
              <div className="flex justify-end">
                <span className={cn(
                  "text-xs font-medium",
              ...
              {permissions.map((p) => (
                ...
                >
                  ...
                  <div className="flex flex-col gap-0.5">
                    ...
                    <span className="text-[11px] text-surface-500 leading-tight">{p.description}</span>
                  </div>
                </button>
              ))}
            </div>
          ...
          <div className="flex items-center gap-2 p-4 rounded-xl bg-amber-50 dark:bg-amber-900/10 text-amber-600 dark:text-amber-400">
            ...
            <p className="text-[11px] font-medium leading-tight">생성 후 공개/비공개 상태는 변경할 수 없습니다.</p>
          </div>
        </form>
      </div>
