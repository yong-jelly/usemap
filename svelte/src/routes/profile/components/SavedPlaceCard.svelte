...
import type { Place } from '$services/types';
	import { ago } from '$utils/date.util';
...
function handleImageError(event: Event) {
  ...
  if (imgElement && imgElement.src) {
    imgElement.src = 'https://placehold.co/400x400?text=NO+IMAGE&font=source-sans-pro';
  ...
  <div class="px-2 mt-2 flex items-start justify-between">
    <div class="flex flex-col">
      <h3 class="font-semibold text-base">{place.name}
        <span class="text-xs text-gray-500"> · {ago(place?.bookmark_metadata.bookmarked_at)}</span>
      </h3>
