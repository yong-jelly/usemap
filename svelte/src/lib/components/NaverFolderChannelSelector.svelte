<script lang="ts">
	...
	// 네이버 폴더 목록
	let channels = $state<NaverFolder[]>([]);
	...
	async function fetchChannels() {
		...
		try {
			const { data, error } = await supabase.rpc('v1_get_nfolder_channels');
	...
	function handleChannelSelect(channelId: string) {
		// 전체 선택 해제
		isAllSelected = false;
		console.log('handleChannelSelect', channelId);
		// 이미 선택된 채널이면 선택 해제
			...
			>
				<BottomSheet.Handle>
					...
					<div class="border-b border-gray-200 px-4 py-3 dark:border-neutral-700">
						<h3 class="text-center text-lg font-medium">네이버 폴더 선택</h3>
						<p class="mt-1 text-center text-sm text-gray-500 dark:text-gray-400">
							원하는 폴더를 선택해주세요 (다중 선택 가능)
						</p>
					</div>
				</BottomSheet.Handle>
				<BottomSheet.Content
