...
import type { ExchangeSettingCreateRequestDto, ExchangeSettingListRequestDto, ExchangeSettingUpdateRequestDto } from '../dto/admin.exchange.setting.dto';

export const adminExchangeService = {
    getList:async (dto: ExchangeSettingListRequestDto) => {
        // console.log('dto:', dto);
        const query = Object.entries(dto)
            ...
            .join('&');
        try {
            return await apiClient.get(API_ENDPOINTS.ADMIN.EXCHANGE.LIST(dto.page, query));
        } catch (error) {
    ...
    get:async (id: string) => {
        try {
            return await apiClient.get(API_ENDPOINTS.ADMIN.EXCHANGE.GET(id));
        } catch (error) {
            return error as ErrorDto;
        }
    },
    create:async (dto: ExchangeSettingCreateRequestDto) => {
        try {
            return await apiClient.post(API_ENDPOINTS.ADMIN.EXCHANGE.CREATE, dto);
        } catch (error) {
            return error as ErrorDto;
        }
    },
    update:async (id: string, dto: ExchangeSettingUpdateRequestDto) => {
        try {
            return await apiClient.put(API_ENDPOINTS.ADMIN.EXCHANGE.UPDATE(id), dto);
        } catch (error) {
            return error as ErrorDto;
        }
    },
