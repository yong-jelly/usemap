-- =====================================================
-- 082_optimize_v5_get_my_feed.sql
-- v4_get_my_feed 성능 최적화 버전
-- 
-- 개선 사항:
--   1. Early LIMIT: 최신순 정렬 + 가격 필터 없을 때 먼저 중복 제거 후 LIMIT 적용
--   2. MATERIALIZED CTE: CTE 재계산 방지
--   3. calculate_menu_avg_price 1회 호출 (가격 필터 재사용)
--   4. comment 컬럼명 충돌 해결 (fi_comment alias 사용)
-- 
-- 성능 비교 (user_id: b75408a1-c1cf-43b6-b6f1-3b7288745b62 기준):
--   v3_get_my_feed: 891ms
--   v5_get_my_feed (Fast Path): 155ms (약 95% 개선)
-- 
-- 인자:
...
-- 
-- 실행 방법:
--   psql "postgresql://postgres.xyqpggpilgcdsawuvpzn:ZNDqDunnaydr0aFQ@aws-0-ap-northeast-2.pooler.supabase.com:5432/postgres" -f docs/sql/082_optimize_v5_get_my_feed.sql
-- =====================================================

DROP FUNCTION IF EXISTS public.v5_get_my_feed(INT, INT, INT, INT, TIMESTAMPTZ, DOUBLE PRECISION, DOUBLE PRECISION);
DROP FUNCTION IF EXISTS public.v5_get_my_feed(INT, INT, INT, INT, TIMESTAMPTZ, VARCHAR, DOUBLE PRECISION, DOUBLE PRECISION);
CREATE OR REPLACE FUNCTION public.v5_get_my_feed(
...
BEGIN
    ...
    -- =========================================================
    -- Slow Path: 가격 필터 있음 → 전체 스캔 후 필터링
    -- =========================================================
    ELSE
        RETURN QUERY
        WITH 
        ...
        -- 중복 제거 (정렬 기준에 따라 대표 행 선택)
        deduplicated_feed AS (
            SELECT DISTINCT ON (pid) *
            FROM filtered_feed
            ORDER BY pid,
                CASE WHEN p_sort_by = 'distance' THEN dist_m END ASC NULLS LAST,
                CASE WHEN p_sort_by = 'recent' OR p_sort_by IS NULL THEN added_time END DESC
        ),
        ...
        target_feed AS MATERIALIZED (
            ...
            ORDER BY 
                ...
                CASE WHEN p_sort_by = 'recent' OR p_sort_by IS NULL THEN df.added_time END DESC
            LIMIT p_limit OFFSET p_offset
        ),
        ...
        ORDER BY 
            ...
            CASE WHEN p_sort_by = 'recent' OR p_sort_by IS NULL THEN tf.added_time END DESC;
    END IF;
END;
$$;

COMMENT ON FUNCTION public.v5_get_my_feed IS '내 피드 조회 v5 - 성능 최적화 버전 (Fast Path: Early LIMIT, MATERIALIZED CTE)';
GRANT EXECUTE ON FUNCTION public.v5_get_my_feed TO authenticated;
