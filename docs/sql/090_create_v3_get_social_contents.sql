-- =====================================================
-- 090_create_v3_get_social_contents.sql
-- 소셜(인스타그램, 쓰레드) 맛집을 지역별로 그룹화하여 조회
-- 
-- 인자:
--   @p_service: 필터링할 서비스 (instagram, threads 등)
--   @p_limit: 조회할 지역 개수 (기본 20)
--   @p_offset: 페이징 오프셋
-- 
-- 실행 방법:
--   psql "postgresql://postgres.xyqpggpilgcdsawuvpzn:ZNDqDunnaydr0aFQ@aws-0-ap-northeast-2.pooler.supabase.com:5432/postgres" -f docs/sql/090_create_v3_get_social_contents.sql
-- =====================================================

DROP FUNCTION IF EXISTS public.v3_get_social_contents(text, integer, integer);
CREATE OR REPLACE FUNCTION public.v3_get_social_contents(
...
BEGIN
    ...
    region_places AS (
        SELECT 
            ...
            pf.title,
            pf.content_url,
            pf.metadata->>'service' as service,
            pf.published_at,
...
COMMENT ON FUNCTION public.v3_get_social_contents IS '소셜 맛집을 지역별로 그룹화하여 조회 (v3 - 성능 최적화: 타겟 지역 먼저 확정, 배치 조회)';
GRANT EXECUTE ON FUNCTION public.v3_get_social_contents TO authenticated, anon;
