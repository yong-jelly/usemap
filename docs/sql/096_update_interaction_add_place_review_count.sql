-- =====================================================
-- 096_update_interaction_add_place_review_count.sql
-- v1_common_place_interaction 및 관련 함수에 place_review_count 추가
-- tbl_place_user_review 테이블에서 숨김(is_private=true) 및 삭제(is_active=false)를 제외한 리뷰 개수를 집계합니다.
-- 
-- 실행 방법:
--   psql "postgresql://postgres.xyqpggpilgcdsawuvpzn:ZNDqDunnaydr0aFQ@aws-0-ap-northeast-2.pooler.supabase.com:5432/postgres" -f docs/sql/096_update_interaction_add_place_review_count.sql
-- =====================================================
...
BEGIN
    ...
    -- tbl_place_user_review에서 전체 리뷰 개수 집계 (비공개 리뷰 포함, 061_add_place_reviews_count_to_interaction.sql 호환성 유지)
    SELECT count(*) INTO v_place_reviews_count FROM public.tbl_place_user_review WHERE place_id = p_place_id AND is_active = true;
    
    -- tbl_place_user_review에서 공개된 리뷰 개수만 집계 (숨김/삭제 제외)
    SELECT count(*) INTO v_place_review_count FROM public.tbl_place_user_review 
    ...
    RETURN jsonb_build_object(
        ...
        'place_review_count', COALESCE(v_place_review_count, 0),
...
COMMENT ON FUNCTION public.v1_common_place_interaction IS '장소별 통합 상호작용 정보(좋아요, 저장, 댓글, 리뷰 수 등)를 조회합니다. place_review_count는 공개된 리뷰만 카운트합니다.';
COMMENT ON FUNCTION public.v1_get_place_details IS '장소 상세 정보를 위한 통합 인터랙션 정보를 조회합니다. place_review_count는 공개된 리뷰만 카운트합니다.';

GRANT EXECUTE ON FUNCTION public.v1_common_place_interaction TO authenticated, anon;
GRANT EXECUTE ON FUNCTION public.v1_get_place_details TO authenticated, anon;
