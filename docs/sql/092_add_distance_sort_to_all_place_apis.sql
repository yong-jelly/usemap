-- =====================================================
-- 092_add_distance_sort_to_all_place_apis.sql
-- 모든 장소 목록 API에 거리순 정렬 기능 추가
-- 
-- 수정 대상:
--   1. v1_get_folder_places (맛탐정 폴더)
--   2. v3_get_places_by_naver_folder (네이버 폴더)
--   3. v3_get_places_by_youtube_channel (유튜브 채널)
--   4. v3_get_places_by_community_region (커뮤니티 지역)
--   5. v3_get_places_by_social_region (소셜 지역)
--   6. v3_get_places_by_region (통합 지역)
-- 
-- 변경 사항:
--   - p_sort_by TEXT 파라미터 추가 ('recent' | 'distance')
--   - p_user_lat, p_user_lng DOUBLE PRECISION 파라미터 추가
--   - 거리순: Haversine 거리 계산 후 ORDER BY distance ASC (거리 제한 없음)
--   - 반환에 distance_meters 컬럼 추가
--
-- 실행 방법:
--   psql "postgresql://postgres.xyqpggpilgcdsawuvpzn:ZNDqDunnaydr0aFQ@aws-0-ap-northeast-2.pooler.supabase.com:5432/postgres" -f docs/sql/092_add_distance_sort_to_all_place_apis.sql
-- =====================================================
...
DROP FUNCTION IF EXISTS public.v1_get_folder_places(VARCHAR, INT, INT, BOOLEAN, TEXT, DOUBLE PRECISION, DOUBLE PRECISION);
...
BEGIN
    ...
    SELECT 
        ...
        fp.comment,
        CASE 
            WHEN p_sort_by = 'distance' AND p_user_lat IS NOT NULL AND p_user_lng IS NOT NULL
                 AND pl.x IS NOT NULL AND pl.y IS NOT NULL
                 AND pl.x ~ '^[0-9.-]+$' AND pl.y ~ '^[0-9.-]+$'
    ...
    ORDER BY
        CASE WHEN p_sort_by = 'distance' AND p_user_lat IS NOT NULL AND p_user_lng IS NOT NULL
        THEN (6371 * acos(LEAST(1.0, GREATEST(-1.0, 
            cos(radians(p_user_lat)) * cos(radians(pl.y::double precision)) * 
            cos(radians(pl.x::double precision) - radians(p_user_lng)) + 
            sin(radians(p_user_lat)) * sin(radians(pl.y::double precision))
...
DROP FUNCTION IF EXISTS public.v3_get_places_by_naver_folder(BIGINT, INT, INT, BOOLEAN, TEXT, DOUBLE PRECISION, DOUBLE PRECISION);
...
BEGIN
    ...
    folder_places AS (
        ...
        ORDER BY
            CASE WHEN p_sort_by = 'distance' THEN 1 ELSE 0 END ASC,
            CASE WHEN p_sort_by = 'distance' AND p_user_lat IS NOT NULL AND p_user_lng IS NOT NULL
                      AND p.x IS NOT NULL AND p.y IS NOT NULL
                      AND p.x ~ '^[0-9.-]+$' AND p.y ~ '^[0-9.-]+$'
                THEN (6371 * acos(LEAST(1.0, GREATEST(-1.0, 
    ...
    ORDER BY
        CASE WHEN p_sort_by = 'distance' THEN fp.dist_m ELSE NULL END ASC NULLS LAST,
        fp.visitor_reviews_score DESC NULLS LAST,
        fp.visitor_reviews_total DESC NULLS LAST;
END;
$$;
...
DROP FUNCTION IF EXISTS public.v3_get_places_by_youtube_channel(TEXT, INT, INT, BOOLEAN, TEXT, DOUBLE PRECISION, DOUBLE PRECISION);
...
BEGIN
    ...
    places_with_dist AS (
        ...
        JOIN tbl_place p ON up.pid = p.id
    ),
    paged_places AS (
        SELECT pd.pid, pd.pub_at, pd.dist_m
        FROM places_with_dist pd
        ORDER BY
            CASE WHEN p_sort_by = 'distance' THEN pd.dist_m ELSE NULL END ASC NULLS LAST,
            pd.pub_at DESC
        LIMIT p_limit OFFSET p_offset
    ),
    channel_places AS (
        SELECT 
            pp.pid, pp.pub_at, pp.dist_m,
            p.id, p.name, p.group1, p.group2, p.group3, p.road, p.category,
            p.road_address, p.address, p.phone, p.visitor_reviews_total, p.visitor_reviews_score,
            p.x, p.y, p.images, p.static_map_url, p.keyword_list, p.visitor_review_medias_total,
            p.menus, p.place_images, p.updated_at as place_updated_at, p.created_at as place_created_at,
    ...
    ORDER BY
        CASE WHEN p_sort_by = 'distance' THEN cp.dist_m ELSE NULL END ASC NULLS LAST,
        cp.pub_at DESC;
END;
$$;
...
DROP FUNCTION IF EXISTS public.v3_get_places_by_community_region(TEXT, TEXT, INT, INT, BOOLEAN, TEXT, DOUBLE PRECISION, DOUBLE PRECISION);
...
DROP FUNCTION IF EXISTS public.v3_get_places_by_social_region(TEXT, TEXT, INT, INT, BOOLEAN, TEXT, DOUBLE PRECISION, DOUBLE PRECISION);
...
BEGIN
    ...
    place_interactions AS (
        SELECT pd.pid as place_id, COALESCE(lk.liked_count, 0) as liked_count, COALESCE(sv.saved_count, 0) as saved_count, COALESCE(cm.comment_count, 0) as comment_count, COALESCE(rv.reviews_count, 0) as reviews_count, COALESCE(my_lk.is_liked, false) as is_liked, COALESCE(my_sv.is_saved, false) as is_saved, COALESCE(my_cm.is_commented, false) as is_commented, COALESCE(my_rv.is_reviewed, false) as is_reviewed
        FROM places_with_data pd
        LEFT JOIN LATERAL (SELECT count(*)::int as liked_count FROM public.tbl_like WHERE liked_id = pd.pid AND liked_type = 'place' AND liked = true) lk ON true
        LEFT JOIN LATERAL (SELECT count(*)::int as saved_count FROM public.tbl_save WHERE saved_id = pd.pid AND saved_type = 'place' AND saved = true) sv ON true
        LEFT JOIN LATERAL (SELECT count(*)::int as comment_count FROM public.tbl_comment_for_place WHERE business_id = pd.pid AND is_active = true) cm ON true
    ...
    place_experiences AS (
        SELECT pd.pid as place_id, COALESCE(v.visit_count, 0) as visit_count, v.last_visited_at
        FROM places_with_data pd
        LEFT JOIN LATERAL (SELECT count(*)::int as visit_count, max(visited_at) as last_visited_at FROM public.tbl_visited WHERE user_id = v_user_id AND place_id = pd.pid) v ON true
    ),
    place_features AS (
        SELECT pd.pid as place_id, COALESCE(pf.features, '[]'::jsonb) as features
        FROM places_with_data pd
        LEFT JOIN LATERAL (
            SELECT jsonb_agg(to_jsonb(af.*)) as features FROM (
                SELECT pf.id::text as id, pf.title, pf.status, pf.user_id::text, pf.metadata, pf.created_at, pf.updated_at, pf.content_url, pf.is_verified, pf.published_at, pf.platform_type
                FROM public.tbl_place_features pf WHERE pf.place_id = pd.pid AND pf.status = 'active'
                UNION ALL
                SELECT nf.folder_id::text, nf.name, 'active', NULL::text, jsonb_build_object('url', NULL, 'title', nf.name, 'domain', 'naver', 'description', nf.memo), nf.created_at, nf.updated_at, NULL, false, nf.created_at, 'folder'
                FROM public.tbl_naver_folder_place nfp JOIN public.tbl_naver_folder nf ON nfp.folder_id = nf.folder_id WHERE nfp.place_id = pd.pid
                UNION ALL
                SELECT f.id, f.title, 'active', f.owner_id::text, jsonb_build_object('url', '/folder/' || f.id, 'title', f.title, 'domain', 'feature', 'description', f.description), f.created_at, f.updated_at, '/folder/' || f.id, false, f.created_at, 'public_user'
                FROM public.tbl_folder_place fp JOIN public.tbl_folder f ON fp.folder_id = f.id WHERE fp.place_id = pd.pid AND fp.deleted_at IS NULL AND f.permission = 'public' AND f.is_hidden = FALSE
            ) af
        ) pf ON true
    )
    SELECT pd.pid::VARCHAR as place_id,
        jsonb_build_object(
            'id', pd.id, 'name', pd.name, 'group1', pd.group1, 'group2', pd.group2, 'group3', pd.group3,
            'road', pd.road, 'category', pd.category, 'road_address', pd.road_address, 'address', pd.address,
            'phone', pd.phone, 'visitor_reviews_total', pd.visitor_reviews_total, 'visitor_reviews_score', pd.visitor_reviews_score,
            ...
            'is_franchise', pd.is_franchise, 'conveniences', pd.conveniences, 'homepage', pd.homepage,
            'interaction', jsonb_build_object(
                'place_liked_count', pi.liked_count, 'place_saved_count', pi.saved_count, 'is_liked', pi.is_liked, 'is_saved', pi.is_saved,
                'place_comment_count', pi.comment_count, 'place_reviews_count', pi.reviews_count, 'is_commented', pi.is_commented, 'is_reviewed', pi.is_reviewed,
                'visit_count', pe.visit_count, 'last_visited_at', pe.last_visited_at
            ),
            'features', pfeat.features,
...
DROP FUNCTION IF EXISTS public.v3_get_places_by_region(TEXT, TEXT, INT, INT, BOOLEAN, TEXT, DOUBLE PRECISION, DOUBLE PRECISION);
...
BEGIN
    ...
    all_source_places AS (
        ...
        FROM tbl_place_features pf JOIN tbl_place p ON pf.place_id = p.id
        ...
        FROM tbl_naver_folder_place nfp JOIN tbl_naver_folder f ON nfp.folder_id = f.folder_id JOIN tbl_place p ON nfp.place_id = p.id
        ...
        FROM tbl_folder_place fp JOIN tbl_folder f ON fp.folder_id = f.id JOIN tbl_place p ON fp.place_id = p.id
    ...
    unique_places AS (
        SELECT DISTINCT ON (asp.place_id)
            asp.place_id as pid, asp.published_at as pub_at, asp.src, asp.title
        FROM all_source_places asp
        WHERE (
            NOT p_visited_only 
            OR (v_user_id IS NOT NULL AND EXISTS (
              SELECT 1 FROM tbl_visited v WHERE v.user_id = v_user_id AND v.place_id = asp.place_id
            ))
        )
    ...
    paged_places AS (
        SELECT pd.pid, pd.pub_at, pd.src, pd.title, pd.dist_m
        FROM places_with_dist pd
        ORDER BY
            CASE WHEN p_sort_by = 'distance' THEN pd.dist_m ELSE NULL END ASC NULLS LAST,
            pd.pub_at DESC
    ...
    places_with_data AS (
        SELECT 
            pp.pid, pp.pub_at, pp.src, pp.title as source_title, pp.dist_m,
            p.id, p.name, p.group1, p.group2, p.group3, p.road, p.category,
            p.road_address, p.address, p.phone, p.visitor_reviews_total, p.visitor_reviews_score,
            p.x, p.y, p.images, p.static_map_url, p.keyword_list, p.visitor_review_medias_total,
            p.menus, p.place_images, p.updated_at as place_updated_at, p.created_at as place_created_at,
