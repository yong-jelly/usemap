...
> **주의**: `tbl_folder_place`, `tbl_folder_review` 등이 `tbl_place`를 FK로 참조하므로, 아래 **연관 테이블 포함 전체 정리**를 사용할 것. 이 쿼리만 단독 실행 시 FK 제약 위반 발생함.
...
-- 아래 '연관 테이블 포함 전체 정리' 섹션 사용 권장 (FK 제약 때문)
```
